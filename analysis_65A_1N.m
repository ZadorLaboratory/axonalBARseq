...
% Load ctxML/AP/DepthPrctile image without definded fold-in areas
[ctxML, ctxAP, ctxDepthPrctile] = TBS.loadMLAPDim(directory.main,refSetting);
...
% Exclude soma location outlier
I = any(mlapdSoma,2);
I = find(I);
% Has an outlier in either axis
TF = isoutlier(mlapdSoma(I,1:2),1);
TF = any(TF,2);
I = I(TF);
mlapdSoma(I,:) = 0;
xyzSoma(I,:) = 0;
disp(['Excluding soma location with ML-AP outliers: ',num2str(numel(I))]);
...
flatmapYLim = [floor(min(flatmapYLim)), ceil(max(flatmapYLim))];
...
CF = TBS.groupCTPT(xyzDot,regVoxel{3},regVoxel{4},regVoxel{5});

% thal-mb+ cells
% (Already thresholding for minimum rolony in midbrain, i.e 3)
TF = cellfun(@(X) TBS.regVoxelTF(X,regVoxel{5}),xyzDot,'UniformOutput',false);
TF = cellfun(@any,TF);
TF = isnan(CF) & TF;
...
TF = cellfun(@(X) TBS.regVoxelTF(X,regVoxel{2}),xyzDot,'UniformOutput',false);
% With rolony more than minimum counts
TF = cellfun(@sum,TF);
TF = TF >= regionMinCount;
ITc = IT & TF;
...
TF = cellfun(@(X) sum(X),TF) >= regionMinCount;
...
center = [7050 6950];  
% center = [7050 6950; 7500 6450; 6187 6668];  
...
% regVoxelTF
TF = vertcat(xyzDot{:});
TF = TBS.regVoxelTF(TF,regVoxel2>0);
...
for i = 1:numel(regVoxel)
    TF = cellfun(@(X) TBS.regVoxelTF(X,regVoxel{i}),xyzDot,...
        'UniformOutput',false);
...
% 3. Sorting using ctxC counts
for i = 1:max(idx)
    row = idx == i;
    iStat = stat(row,:);
    
    iStat = sortrows(iStat,regCol.ctxC,'descend');
    stat(row,:) = iStat;
end
...
figure; scatter(xyBin(:,1),xyBin(:,2),30,idx,'filled');
...
% Manually checked: 
% Disimilar: 372: 1   1   2   2   1   1   3   2   3   2   4   1   1   4   3   3   1
% Similar: 6838: 3   4   3   3   3   4   1   2   3   2   2   3   1   1   2   3   4
I = [372 6838];
...
% Manually pick BC ID
% I = [1 530 390 433 5308 2089];
I = [2089 1 433];

% Mannually check projection pattern
% for i = 1:size(mlapdDot2,1)
%     if ~group(i,3) || ~any(d{2}(i,:)) || ~any(d{1}(i,:))
%         continue
%     end
...
im = TBS.xyzv2im(size(annoMap),xyz(TF,:),c(TF));
...
roi = cellfun(@(X) TBS.regVoxelTF(X,roi),xyzDot2,'UniformOutput',false);
